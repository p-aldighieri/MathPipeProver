...
4. Verify the CDP browser at port `{{CDP_PORT}}` is up and the ChatGPT project at `{{CHATGPT_PROJECT_URL}}` is open in it. Verify the composer pill reads "6 Pro" (the Pro target — GPT-6, top Power level; legacy names "Sol Pro", "Extended Pro") — if not, fix via `/set-model-extended` (see CLAUDE.md §Model Configuration — CRITICAL; the wrong pill silently swaps to a weaker model).
...
9. After each Pro (or Deep Research) submission, record the chat URL and response file path, and start the chat watcher (`scripts/chatgpt_browser_agent/wait_chat_done.mjs`) as a background job: its exit — with the answer already written — wakes you for the next role (see CLAUDE.md §Waiting on a submission). Use `/inspect-chat` for one-shot status reads and `/recover-chat` to harvest a completed chat that wasn't captured. `/heartbeat <interval>` remains a long-interval fallback loop for very long unattended runs.
