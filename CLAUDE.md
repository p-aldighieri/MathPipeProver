...
- **Parallelize carefully — sequence by default.** ChatGPT throttles bursts ("You're making requests too quickly", 2026-09-21, after eight submissions with multi-file attachments in ~15 minutes while five watchers reloaded chat pages). Default to **one chat in flight and one watcher at a time**; the submit scripts also enforce a minimum gap between submissions (`MPP_MIN_SUBMIT_GAP_SECONDS`, default 180, see `lib/throttle.mjs`). Run two chats in parallel only when they are genuinely independent and the user has not asked for sequential work. Attach only the files a role needs. Keep an explicit log of which run owns which durable project source vs. which prompt-specific attachment, so concurrent runs do not collide on shared sources. When in doubt, sequence.
...
| `lib/throttle.mjs` | Submission pacing: every submit waits until `MPP_MIN_SUBMIT_GAP_SECONDS` (default 180) have passed since the last one on the same CDP port. |
