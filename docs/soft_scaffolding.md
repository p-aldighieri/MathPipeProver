...
2. **Re-attack from the searcher.** Loop back to the searcher and pass the dossier as an *explicit* input (same handling as `literature.md` — a file the searcher must read, not merely have available in the background). The searcher proposes a fresh route set that avoids the refuted routes and targets the recorded obstruction.
   ...
   **Optional on attempt ≥2: `/search-council`.** When the regular searcher's first re-attack doesn't surface a meaningfully new route shape — when the searcher keeps converging on refinements of the prior approach instead of a genuine pivot — escalate to `/search-council`. The skill fans out the same packet (objective + paper + dossier + prior-routes) to four independent council members in parallel, **one per model architecture** (1 Codex thinking-high + 1 latest Gemini Pro + 1 Claude Opus + 1 ChatGPT Extended Pro), preserves four immutable memos, and hands them to the regular searcher in selection mode (pick 2-4 routes across all four memos, rank, recommend, cite memo provenance). Cross-architecture diversity (Codex + Gemini + Opus + Extended Pro) surfaces moves that any single architecture wouldn't propose — verified live 2026-05-27 with the canonical separately-convex example, where Opus produced an epigraph-lifting route neither other member proposed. Cost is ~3× a single search; opt-in only, not default on every re-attack.
