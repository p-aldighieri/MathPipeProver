...
5. **Wait + harvest.** Use `/inspect-chat`, `/recover-chat`, or manual polling. For unattended runs, `/heartbeat <interval>` will wake the orchestrator periodically to advance the pipeline. When the response is in, parse the leading `lean_structure` fenced block to extract `object_count`, `lemma_count`, etc. If the block is missing or malformed, treat as a failed submission and re-submit with a clarifying note (the model occasionally drops the parseable block; re-prompting fixes it).
