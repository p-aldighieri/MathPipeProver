...
5. **Wait + harvest.** Run the chat watcher (`wait_chat_done.mjs`, as `/submit-role` step 5 describes) as a background job; its exit means the response file is written. `/inspect-chat` and `/recover-chat` remain available for one-shot checks and manual harvests. When the response is in, parse the leading `lean_structure` fenced block to extract `object_count`, `lemma_count`, etc. If the block is missing or malformed, treat as a failed submission and re-submit with a clarifying note (the model occasionally drops the parseable block; re-prompting fixes it).
